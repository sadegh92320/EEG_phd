...
class ImportPhdSimulator(DataImport):
    ...
    def get_config(self):
        self.config = r"MAE_pretraining\info_dataset\im_lab.yaml"
    
    def import_data(self):
        """Import data from matlab file and segment it according to experiment"""
        data_eeg = []
        path = "D:\EEG_data\pretraining\EEGforSadegh\PhD_Simulator"
        ...
        for file in to_check:
            ...
            part_number = file.split(".")[0].split("_")[2]
            print(part_number)
            with h5py.File(mat_path, "r") as f:
                y = np.array(f["y"])
                eeg = y[:,1:33]
               
                data_eeg.append((part_number, eeg.T))
...
if __name__ == "__main__":
    ...
    data_import = ImportPhdSimulator()
