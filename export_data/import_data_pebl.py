...
class ImportDataPebl(DataImport):
    ...
    def import_data(self):
        ...
        for file in to_check:
            if not str(file).isdigit():
                continue
            mat_path = os.path.join(path, file,f"test_data_{file}.mat")
...
if __name__ == "__main__":
    ...
    data = ImportDataPebl(config=config, mne_process=MNEMethods(config=config))
    data()