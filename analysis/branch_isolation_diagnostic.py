...
def extract_features_branch(model, dataloader, device, branch="both", max_batches=20):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader):
            ...
            # Match Downstream.forward: skip temporal PE when RoPE is enabled.
...
def run_diagnostic(checkpoint_path, data_path="downstream/data/bci_comp_2a",
                   ...
                   batch_size=32, use_rope=False,
                   max_batches=20, num_classes=None, base_sfreq=250):
    ...
    # Derive num_classes and channel count from the config YAML so we don't
    # silently reuse a BCI-2a config when running on another dataset.
    import yaml as _yaml
    with open(config_path, "r") as f:
        _cfg = _yaml.safe_load(f)
    if num_classes is None:
        num_classes = _cfg.get("num_classes")
        if num_classes is None:
            raise ValueError(
                f"num_classes not provided and not found in {config_path}. "
                f"Pass --num_classes explicitly."
            )
    n_chan_cfg = len(_cfg.get("channel_list", []))

    print(f"Device: {device}")
    print(f"Checkpoint: {checkpoint_path}")
    print(f"Data path: {data_path}")
    print(f"Config: {config_path}  (channels={n_chan_cfg}, num_classes={num_classes})")
    ...
    branch_modes = ["both", "spatial_only", "temporal_only"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_classes", type=int, default=None,
                        help="Override num_classes. If None, read from config YAML.")
    parser.add_argument("--base_sfreq", type=int, default=250,
                        help="Source sampling rate of the dataset (e.g. 256 for FACED, 250 for BCI-2a).")
