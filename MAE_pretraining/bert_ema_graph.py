...
class AdaptiveRiemannEMAGraphBert(pl.LightningModule):
    ...
    def encoder_forward(self, x, channel_list):
        ...
        # EMA-conditioned channel embedding: G_ema row → project → add
        ema_ref = self.ema_graph.G_ema  # (144, 144)
        ema_proj = self.ema_proj(ema_ref)  # (144, enc_dim)
        ema_chan = ema_proj[channel_idx]  # (C, enc_dim)
        ema_chan = ema_chan.unsqueeze(0).expand(B, -1, -1)
        ema_chan = ema_chan.unsqueeze(1).repeat(1, N, 1, 1).view(B, L, -1)

        # Channel identity embedding + EMA relational embedding
        ...
        # Encoder (no EMA reference — EMA is used via embedding, not tangent space)
