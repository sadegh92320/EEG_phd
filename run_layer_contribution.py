"""
Layer Contribution Analysis — Run with checkpoint on BCI-IV 2a.

Tests whether each layer's attention learns discriminative spatial
structure that gets diluted by the residual stream.

Usage:
    python run_layer_contribution.py --checkpoint /path/to/checkpoint.ckpt
"""
import sys
import os
import torch
...
def run(checkpoint_path, data_path="downstream/data/bci_comp_2a",
        ...
        num_layers=8, batch_size=32, max_batches=None, sample_cap=100):
    ...
    # ── 3. Run analysis ──
    print(f"[3/3] Running layer contribution analysis ({num_layers} layers)...")
    print()

    results = run_layer_contribution(
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Layer Contribution Analysis")
    ...
    parser.add_argument("--sample_cap", type=int, default=100,
                        help="Max samples per class to store")
